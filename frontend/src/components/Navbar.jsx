...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 font-jakarta ${scrolled ? 'shadow-sm bg-white/90 backdrop-blur-md border-b border-slate-200 py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <Link to={isAdmin ? "/admin" : "/"} className="text-4xl font-extrabold tracking-tighter text-slate-900 cursor-pointer">
        ...
        <div className="flex gap-4 items-center">
          ...
          {isLoggedIn ? (
            <button
              ...
              className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-2.5 rounded-full font-semibold transition-all shadow-md cursor-pointer">
              Logout
            </button>
          ) : (
            <>
