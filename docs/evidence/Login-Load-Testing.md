...
Three profiles were run against a live local instance of the API (Postgres + Kafka via
`docker-compose`, API via `dotnet run`), each against the same seeded Student account
